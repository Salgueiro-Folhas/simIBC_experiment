...
class ServerCore(object):
	...
	def __init__(self, my_port = 50082, core_node_host=None, core_node_port=None, crm = None):
		...
		if PoS_Sim_Time != None:
			print("==========================PoS==========")
			self.bb_interval = PoS_Sim_Time
			print("self.bb_interval is[PoS_sim_time]", self.bb_interval)

		elif Random_CHECK_INTERVAL_FLAG == True:
	...
	def __generate_block_with_tp(self):
		...
		if self.flag_stop_block_build is not True:
			if SIM_MODE == "superrelayer":
				print("SIM_MODE is !!!!!superrelayer!!!!!")
				...
				if result_tp == []:
					...
					new_block = self.bb.generate_new_block("", self.prev_block_hash, cross_reference, SIM_MODE, self.block_num)
				...
				if len(new_tp) == 0:
					...
					new_block = self.bb.generate_new_block(new_tp, self.prev_block_hash, cross_reference, "superrelayer", self.block_num)
					self.bm.set_new_block(new_block.to_dict())
					self.prev_block_hash = self.bm.get_hash(new_block.to_dict())
					message_new_block = self.cm.get_message_text(MSG_NEW_BLOCK, json.dumps(new_block.to_dict(), sort_keys=True, ensure_ascii=False))
					self.cm.send_msg_to_all_peer(message_new_block)
			...
			elif SIM_MODE == "relayer": #7# IBCのやつ
				print("SIM_MODE is !relayer!")
				print("/////////////////////////////////////",self.refblock_count) 
				result_tp = self.tp.get_stored_transactions()
				cross_ibc = self.ibc.get_IBC_pool()
				ibc_proof = self.ibc.get_IBC_prof()#7#やり方を変える必要がある。
				print("/////////////////////////////////////", cross_ibc)
				logging.debug("Layer1 croos_reference__" + str(cross_ibc))
				print("check_cross_reference pool")
				self.block_num += 1
				print(self.ibc.check_ref_block_num())
				logging.debug("self.ibc.check_ref_block_num()" + str (self.ibc.check_ref_block_num()))

				if cross_ibc:
					self.ibc.ref_block_number(self.block_num)
					self.refblock_count += 1
					
				if result_tp == []:
					print('Transaction Pool is empty ...')
					#TODO:Create Empty Block
					new_block = self.bb.generate_new_block("", self.prev_block_hash, cross_ibc, ibc_proof, self.block_num)
					self.bm.set_new_block(new_block.to_dict())
					self.prev_block_hash = self.bm.get_hash(new_block.to_dict())
					message_new_block = self.cm.get_message_text(MSG_NEW_BLOCK, json.dumps(new_block.to_dict(), sort_keys=True, ensure_ascii=False))
					self.cm.send_msg_to_all_peer(message_new_block)
					...
					if cross_ibc:
						self.ibc.clear_IBC()
						print("may be。======== reference part clear なにか問題起きてるよmay be。 ======== Tarn to IBC_state → ", cross_ibc)
						self.ibc.myblock_in = True
						print("======== self.ibc.myblock_in ========  → ", self.ibc.myblock_in)

					tp2 = self.ibc.time_stop_phase2()
					if tp2:
						print("Phase2_time:",tp2)
						time = str( "\n Turn" + str(self.ibc.inc)  + " : Phase2: " + str(tp2) + " sec")
						print(time)
						self.Phase2_list.append(time)
				...
				if len(new_tp) == 0:
					...
					new_block = self.bb.generate_new_block(new_tp, self.prev_block_hash, cross_ibc, ibc_proof, self.block_num)
					self.bm.set_new_block(new_block.to_dict())
					self.prev_block_hash = self.bm.get_hash(new_block.to_dict())
					message_new_block = self.cm.get_message_text(MSG_NEW_BLOCK, json.dumps(new_block.to_dict(), sort_keys=True, ensure_ascii=False))
					self.cm.send_msg_to_all_peer(message_new_block)
					...
					if tp2:
						...
						self.Phase2_list.append(time)
				self.Confirmed_block()
				if self.ibc.check_ref_block_num() == self.block_num:
					print("firm block")

			else:
				print("SIM_MODE is None")
		...
		if self.refblock_count == REF_COUNT:
			...
			if self.filesave_flag == True:
				...
				if self.ibc.IBC_time_list:
					filename02 = "IBC_time" + ".txt"
					for i in self.ibc.IBC_time_list:
						with open(filename02, mode='a') as f:
							f.write(str(i) + "\n")
					self.ibc.IBC_time_list.clear()
					self.filesave_flag = False
			else :
		...
		self.bb_timer = threading.Timer(self.bb_interval, self.__generate_block_with_tp)
		print("==========================PoS-"+str(PoS_Sim_Time)+"second==========================")
		self.bb_timer.start()
